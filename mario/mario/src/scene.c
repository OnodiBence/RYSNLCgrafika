...
void init_scene(Scene* scene)
{
	...
	scene->light=0.8f;
	scene->gravity=0.0001f;
	scene->player_velocity=0.5f;
	...
	for(int i=0;i<scene->block_counter;i++){
		...
		for(int k=0;k<scene->block_counter;k++){
			if(k!=i){
				...
				if(scene->blocks[k].is_active==0) continue;
				scene->blocks[i].position.y -=0.02f*scene->blocks[i].move*elapsed_time;
				if(fabs(scene->blocks[i].position.y-scene->blocks[k].position.y)<scene->block_size&&fabs(scene->blocks[i].position.z-scene->blocks[k].position.z)<scene->block_size){
					scene->blocks[i].position.y+=(0.02f * elapsed_time*scene->blocks[i].move);
